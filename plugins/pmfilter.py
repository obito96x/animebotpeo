...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("👨‍💻 ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ", url="https://t.me/i_killed_my_clan")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
...
async def auto_filter(client, msg, spoll=False):
    ...
    anime_info = await get_anime_info(search)

    if anime_info:
        cap = f"🎬 **Name:** {anime_info.get('title', search)}\n"
        cap += f"🔢 **Episodes:** {anime_info.get('episodes', 'Unknown')}\n"
        cap += f"👤 **Character:** {anime_info.get('character', 'N/A')}\n\n"

        description = anime_info.get('synopsis', '')[:200]
        if description:
            cap += f"**Info:** {description}...\n\n"
            
        cap += f"🎥 **Trailer:** {anime_info.get('trailer', 'Not Available')}\n\n"
        cap += "📚 **Your Requested Files 👇**\n\n"

        for f in files:
            cap += f"<b><a href='https://telegram.me/{temp.U_NAME}?start=files_{f.file_id}'> 📁 {get_size(f.file_size)} ▷ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), f.file_name.split()))}\n\n</a></b>"
    ...
    if cover_image:
        try:
            final_msg = await message.reply_photo(photo=cover_image, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await m.delete()
            try:
            ...
            final_msg = await message.reply_text(text=cap, reply_markup=InlineKeyboardMarkup(btn))
            await m.delete()
        ...
        final_msg = await message.reply_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)
        await m.delete()
        try:
            if settings['auto_delete']:
                await asyncio.sleep(DELETE_TIME)
